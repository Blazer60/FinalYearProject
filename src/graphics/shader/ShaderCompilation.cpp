...
namespace graphics
{
    ...
    void ShaderPreprocessor::preprocessInclude(std::string token, ShaderInformation& shaderData)
    {
        ...
        mCurrentPath = shaderData.preprocessInclude(absolutePath);
        walk();
        mCurrentPath = shaderData.path;
    }

    void ShaderPreprocessor::walk()
    {
        if (mAllPaths.count(mCurrentPath) > 0)  // todo: How do we also crash on a cyclic dependancy? Maybe a pending list?
            return;  // We've already processed this file or in the process of.
        mAllPaths.emplace(mCurrentPath);
        ...
        {
            { "include", [&] { preprocessInclude(tokens[1], shaderData); } },

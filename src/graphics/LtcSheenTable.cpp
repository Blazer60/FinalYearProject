...
namespace graphics::sheen
{
    std::array<float, floatCount> data()
    {
        ...
        const std::array<std::array<glm::vec3, tableSize>, tableSize> ltcTable = table();
        for (int i = 0; i < tableSize; ++i)
        {
            for (int j = 0; j < tableSize; ++j)
            {
                result.at(resultIndex++) = ltcTable.at(i).at(j).x;
                result.at(resultIndex++) = ltcTable.at(i).at(j).y;
                result.at(resultIndex++) = ltcTable.at(i).at(j).z;
            }
        }
    ...
    {
        using Vector3f = glm::vec3;  // I'm not refactoring this.

        // Original data from ltc_table_sheen_volume.cpp by Tizian Zeltner, Brent Burley, Matt Jen-Yuan at Walt Disney Studios.
        // https://github.com/tizian/ltc-sheen/tree/master
        return std::array<std::array<glm::vec3, tableSize>, tableSize>
        {
            std::array<glm::vec3, tableSize>
            {
